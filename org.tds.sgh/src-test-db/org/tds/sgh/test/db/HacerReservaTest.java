...
public class HacerReservaTest extends DbTestBase
{
	// curso típico ...........................................................
	...
	public void HacerReservaCursoTipico() throws Exception
	{
		...
		assertTrue("El sistema debe mandar un mail al cliente confirmando la reserva.",
				   mails.size() == 1);
	...
	public void HacerReservaCursoAlternativoConAltaDeCliente() throws Exception
	{
		...
		assertTrue("El sistema no debe devolver clientes ya que no hay clientes registrados.",
				   clientes != null && clientes.size() == 0);

		ClienteDTO clienteX = hacerReservaController.registrarCliente(cliente1);
		
		assertTrue("El cliente registrado debe ser el cliente1.",
				   cliente1.equals(clienteX));
	...
	public void HacerReservaCursoAlternativoSinDisponibilidad() throws Exception
	{
		...
		HotelDTO hotel3 = generarHotelEnPais(hotel1.getPais());
		
		TipoHabitacionDTO tipoHabitacion1 = generarTipoHabitacion();
		TipoHabitacionDTO tipoHabitacion2 = generarTipoHabitacion();
		
		HabitacionDTO habitacion11 = generarHabitacion(hotel1, tipoHabitacion1);
		HabitacionDTO habitacion12 = generarHabitacion(hotel1, tipoHabitacion2);
		HabitacionDTO habitacion13 = generarHabitacion(hotel1, tipoHabitacion2);
		HabitacionDTO habitacion21 = generarHabitacion(hotel2, tipoHabitacion1);
		HabitacionDTO habitacion31 = generarHabitacion(hotel3, tipoHabitacion1);
		HabitacionDTO habitacion32 = generarHabitacion(hotel3, tipoHabitacion2);
		...
		boolean disponibilidad = hacerReservaController.confirmarDisponibilidad(hotel1, tipoHabitacion1, fecha1, fecha3);
		...
		assertTrue("Los datos de la reserva no son los esperados.",
		...
		clientes = hacerReservaController2.buscarCliente(".*lani.*");
		...
		assertTrue("El hotel no debe tener disponibilidad ya hay una sola habitación del tipo indicado y hay una reserva que interfiere con las fechas indicadas.",
				   !disponibilidad);
		
		
		List<HotelDTO> hoteles = hacerReservaController2.sugerirAlternativas(hotel1.getPais(), tipoHabitacion1, fecha2, fecha4);
		
		assertTrue("El hotel3 es el único que debe ser sugerido como alternativa ya que tiene disponibilidad y está en el mismo país.",
		...
		assertTrue("El sistema debe haber mandado dos mails al cliente ya que se han hecho dos reservas en total.",
				   mails.size() == 2);
		
		assertTrue("El destinatario de los mails debe ser el cliente1.",
